...
| 格式 | MIME | 扩展名 | 备注 |
|------|------|--------|------|
| PNG | `image/png` | `.png` | 默认格式，无损，推荐 |
| JPG | `image/jpeg` | `.jpg` | 有损压缩，quality=0.92 |

默认导出 PNG。PNG 和 JPG 均支持全部背景模式（见 3.2）。
...
| 白色 | 纯白底 | 导出前通过 `gridVisible=false` 隐藏网格层，Stage 底色为白 | PNG / JPG |

默认模式: **网格**（与画布编辑时视觉一致）。

两种模式均适用于 PNG 和 JPG，无格式限制。白色背景通过切换 V3A 中 `useCanvasStore.gridVisible` 字段实现（导出前设为 false，导出后恢复为 true），见 4.2 节。
...
取消当前选中 (setSelectedId(null))  ← 隐藏 Transformer
    │
    ▼
按背景模式调整: 网格模式？ ──是──▶ 什么都不做
                  ──否──▶ 设置 gridVisible = false（隐藏 GridBackground Layer）
    │
    ▼
...
创建 <a download="whiteboard-..."> 触发浏览器下载
    │
    ▼
恢复导出前状态: 设置 gridVisible = true（恢复为导出前值）
    │
    ▼
...
export type BackgroundMode = 'grid' | 'white'
...
该函数：
1. 从 store 获取 Stage：`const stage = useCanvasStore.getState().getStage()`
2. **错误处理**: 若 `stage === null`，显示 Toast "画布未就绪，请稍后重试" 并 return（不静默失败）
3. 保存当前 `gridVisible` 状态（`useCanvasStore.getState().gridVisible`）
4. 白色背景模式下：`useCanvasStore.setState({ gridVisible: false })`，等待 Konva 重绘后导出；网格模式下跳过此步
5. 调用 `stage.toBlob()`
6. 触发浏览器下载
7. 恢复 `gridVisible` 为导出前的值：`useCanvasStore.setState({ gridVisible: <savedValue> })`
8. 成功时 Toast 显示文件名；失败时 Toast "导出失败，请重试"

> **依赖 V3A**: `gridVisible` 字段定义在 V3A 的 `useCanvasStore` viewport 区域。GridBackground 组件通过读取 `gridVisible` 控制自身 Layer 的 `visible` 属性。
...
画布无任何 shape 时仍允许导出，得到纯网格/白色背景图。不做空画布拦截，用户可能有只导出背景的需求。
...
| `src/components/GridBackground.tsx` | 读取 `useCanvasStore.gridVisible` 控制 Layer `visible` 属性（依赖 V3A 中该字段的实现） |
...
- [ ] 导出不发送任何网络请求
- [ ] 导出后画布编辑状态无损（grid 可见性恢复、选中态恢复可接受的丢失）
- [ ] 画布未就绪时（stage 为 null）显示 Toast "画布未就绪，请稍后重试"
